...
export default function StepsSection() {
    const steps = useSelector(state => state.transport.steps);
    const stepsContent = useSelector(state => state.transport.stepsContent);
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10 relative">
                    ...
                    {steps.map((item, i) => (
                        <div key={i} className="relative z-10 flex flex-col items-center group">
                            ...
                            <div className="text-amber-500 font-black italic text-xl mb-2">{stepsContent.stepPrefix || 'STEP'} {item.step}</div>
