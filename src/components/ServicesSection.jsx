            ...
            <div className="max-w-[85rem] mx-auto px-6 lg:px-12 relative z-10">
                <div className="text-center mb-10 md:mb-16">
                    <div className="inline-flex items-center gap-2 mb-4 px-4 py-2 border border-amber-500/30 rounded-full bg-amber-500/10">
                        <span className="text-amber-500 font-bold tracking-[0.2em] capitalize text-xs">{(servicesContent.badge || 'Our Services').toLowerCase()}</span>
                    </div>
                    {isHero ? (
                        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-black italic capitalize text-white tracking-tighter">
                            {(servicesContent.title || 'Premium').toLowerCase()} <span className="text-amber-500">{(servicesContent.highlight || 'Solutions').toLowerCase()}</span>
                        </h1>
                    ) : (
